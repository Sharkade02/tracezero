...
| 21 | Localisation | IN_PROGRESS | — | **Infra complète** (`fr/en/de/es` à chaud, `ILocalizationService`, `Localizer`, sélecteur Paramètres, persistance). **Tout le texte statique des 16 pages localisé en 4 langues** (titres nav rafraîchis live, en-têtes/sous-titres/boutons/sections/libellés de chaque page). **Reste** : messages dynamiques générés dans les VM (StatusMessage/ResultMessage/toasts), **descriptions de règles** (Engine) et messages d'erreur (§31) → `KNOWN_LIMITATIONS.md` |

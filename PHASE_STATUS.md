...
| 19 | Installateur / Portable | IN_PROGRESS | 2 ✅ | **Mode portable** + **installeur EXE** livrés et testés. Portable : marqueur `tracezero.portable` → données dans `<exe>\Data`. Installeur : `build/scripts/publish-installer.ps1` + `build/installer/TraceZero.iss` (Inno Setup) → `TraceZeroSetup-<v>.exe`, per-user sans admin, raccourci menu Démarrer, désinstallation propre (install/lancement/désinstall vérifiés). **Manifeste MSIX** `build/msix/Package.appxmanifest`. **Reste** : **signature** (certificat requis, cf. `docs/signpath-application.md`), assets MSIX, `.ico` dédiée, MSI optionnel |
